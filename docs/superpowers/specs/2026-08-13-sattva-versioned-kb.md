...
**Notion hub:** https://app.notion.com/p/3bbe8d8c60c78198b879e272e52dd5d4
...
1. The KB is a workspace top-level page, sibling of LifeOS, not a child of it.
...
├── Closed teamspace (target) ← restore “Sattva Brokers” or recreate “Trilok Ventures - Sattva Brokers”
│   └── This KB
└── Nextcloud vault ← RED + AMBER files
```

**Interim (now):** the KB is already a top-level private page (empty ancestor path), sibling of LifeOS. Treat as Ninad-only. Do not invite sales, investors, or guests.

**Target:** restore trashed teamspace `Sattva Brokers` (`328e8d8c-60c7-81d3-923b-00423d4f814a`) or create a **closed** teamspace, then **move** this hub into it. Later move the Sattva Brokers operating hub and fabric twins out from under LifeOS so sharing the KB cannot walk into personal pages.
...
One sibling hub. Link existing LifeOS / Sattva pages; do not clone department wikis into empty shells.
...
Working twin of the fabric spec remains under the LifeOS Sattva hub until the teamspace move: https://app.notion.com/p/3bbe8d8c60c7816ba0def605bf847c5a
...
Existing fabric twins under LifeOS do not yet carry SHAs. Pin them as catalog rows after this spec lands (fabric spec SHA `630787f1ca9ca485464544aef92c52ebb8700a47` is already seeded).
...
Keep in LifeOS: personal cadence, scratch notes, department databases until they are unlinked from the personal tree.

---

## 10. Scope

**In scope:** this architecture, the Notion sibling hub already created, catalog/SOP/decision/GREEN databases, access and agent pages.

**Out of scope until a later approved plan:** Compose expansion, GCP, HubSpot, restoring the teamspace via API, rewriting LifeOS department databases, moving the Sattva hub with Notion `<page>` tags from this agent (that **moves** pages).
