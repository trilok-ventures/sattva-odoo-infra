...
- Live POST is operator-only: `sessionStorage.sattva_lead_webhook` + `sessionStorage.sattva_lead_hmac`. Committed default is local validate + preview (no fetch). The HMAC secret must not ship in git or in the static bundle. Live fetch requires `https`, no query/hash, and pathname ending `/webhook/lead-inbound`.
