...
- Static page `docs/superpowers/mocks/inbound-lead.html` (linked from the prototype map and the mocks index). Root `vercel.json` stays `outputDirectory: docs/superpowers/mocks`. No Next.js, no `NEXT_PUBLIC_` fabric URLs, no n8n/Odoo hostname in committed HTML/JS.
...
- Live POST is operator-only: `sessionStorage.sattva_lead_webhook` + `sessionStorage.sattva_lead_hmac`. Committed default is local validate + preview (no fetch). The HMAC secret must not ship in git or in the static bundle. Live fetch requires `https`, no query/hash, and pathname ending `/webhook/lead-inbound`. Do not add a Vercel serverless hop or `NEXT_PUBLIC_` n8n URL to work around CORS; live ingest is operator curl or a browser that can reach the webhook origin.
- `wf.lead.inbound` uses dedicated env `N8N_LEAD_INBOUND_HMAC` (Secret Manager id `n8n-lead-inbound-hmac`). It must not reuse `N8N_WEBHOOK_HMAC`. The webhook node uses `responseMode: onReceived` so the mocks origin never receives Odoo JSON-RPC. The Code node verifies `x-sattva-webhook-hmac`, allowlists keys, rejects nested objects and unknown fields, then calls `sattva.fabric.lead.ingest.create_inbound`. Execution save-data stays `none` (AMBER email in transit).
